...
$query_rs1 = "SELECT * FROM contribution ORDER BY id DESC";
...
<div class="container">
  ...
  <h1>【投稿管理】</h1>
  <h4>这是投稿管理页面，您有<?php $cxsql = "select count(*) from contribution where 'read'=0";$cx = mysql_query($cxsql,$conn);var_dump($cx); ?>条未读投稿</h4>
    <?php if ($totalRows_rs1 > 0) { // Show if recordset not empty ?>
      <table width="800" border="1">
        <tr>
          <td><strong>投稿编号</strong></td>
          <td><strong>投稿人</strong></td>
          <td><strong>方面</strong></td>
          <td><strong>投稿标题</strong></td>
          <td><strong>投稿时间</strong></td>
          <td colspan="2"><strong>操作</strong></td>
          </tr>
        <?php do { ?>
          <tr>
            <td><?php echo $row_rs1['id'];if($row_rs1['read']=="0"){echo "<font color=red>（未读）</font>";}if($row_rs1['release']=="1"){echo "<font color=red>（已发布）</font>";} ?></td>
            <td><?php echo $row_rs1['username']; ?></td>
            <td><?php echo $row_rs1['aspect']; ?></td>
            <td><?php echo $row_rs1['title']; ?></td>
            <td><?php echo $row_rs1['time']; ?></td>
            <td><a href="tg_more.php?id=<?php echo $row_rs1['id']; ?>"><font color="#0099CC">详情</font></a></td>
            <td><a href="#" onclick="javascript:if(confirm('确认删除<?php echo "“$row_rs1[title]”"; ?>吗？')) location='tg_del.php?id=<?php echo $row_rs1['id']; ?>'"><font color="#993300">删除</font></a></td>
          </tr>
