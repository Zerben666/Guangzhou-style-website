...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE contribution SET username=%s, aspect=%s, title=%s, content=%s, `time`=%s, `read`=%s WHERE id=%s",
                       GetSQLValueString($_POST['username'], "text"),
                       GetSQLValueString($_POST['aspect'], "text"),
                       GetSQLValueString($_POST['title'], "text"),
                       GetSQLValueString($_POST['content'], "text"),
                       GetSQLValueString($_POST['time'], "date"),
                       GetSQLValueString($_POST['read'], "text"),
                       GetSQLValueString($_POST['id'], "int"));

  mysql_select_db($database_conn, $conn);
  $Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());

  $updateGoTo = "tg_view.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn, $conn);
$query_rs2 = sprintf("SELECT * FROM contribution WHERE id = %s", GetSQLValueString($colname_rs2, "int"));
...
<div class="container">
  ...
  <form name="form" action="<?php echo $editFormAction; ?>" method="POST" id="form">
    <table width="800" border="0" cellspacing="5">
      <tr>
        <td>投稿人
          <label for="username"></label>
          <input name="username" type="text" id="username" value="<?php echo $row_rs2['username']; ?>" readonly></td>
        <td>投稿时间
          <label for="time"></label>
          <input name="time" type="text" id="time" value="<?php echo $row_rs2['time']; ?>" readonly></td>
      </tr>
      <tr>
        <td>投稿方面
          <label for="aspect"></label>
          <input name="aspect" type="text" id="aspect" value="<?php echo $row_rs2['aspect']; ?>"></td>
        <td>投稿标题
          <label for="title"></label>
          <input name="title" type="text" id="title" value="<?php echo $row_rs2['title']; ?>"></td>
      </tr>
      <tr>
        <td colspan="2">内容
          <label for="content"></label>
          <textarea name="content" cols="90%" rows="5" id="content"><?php echo $row_rs2['content']; ?></textarea></td>
        </tr>
      <tr>
        <td colspan="2"><a href="tg_view.php">返回</a> |          
          <input type="submit" name="button" id="button" value="修改">
          <a href="edit.php?id=<?php echo $row_rs2['id']; ?>">发送到发布编辑</a>          <input name="id" type="hidden" id="id" value="<?php echo $row_rs2['id']; ?>">
          <input name="read" type="hidden" id="read" value="1"></td>
        </tr>
    </table>
