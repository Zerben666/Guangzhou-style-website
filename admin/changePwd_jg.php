...
$query_rs = sprintf("SELECT * FROM `user` WHERE id = %s", GetSQLValueString($colname_rs, "int"));
...
<li><a href="../files/contribution.php<?php echo $url; ?>">投稿</a></li>
  ...
  <?php
  $link = mysqli_connect("localhost","root","","gdgz");
  $username = $_POST['username'];
  $pwd = $_POST['password'];
  $newpwd = $_POST['password1'];
  $newpwd2 = $_POST['password2'];
  
  $cxsql = "select password from user where username='$username' and password='$pwd'";
  @$cxrs = mysqli_query($link,$cxsql);
  @$rs_pwd = mysqli_fetch_assoc($cxrs);
  
  if( (!empty($rs_pwd)) and $newpwd=$newpwd2 ){
	  $upsql = "update user set password='$newpwd' where username='$username'";
	  $cxrs = mysqli_query($link,$upsql);
	  echo "<p>密码修改成功！将在<input type='text' readonly value='10'  size='3' id='time'>秒内跳转到<a href='login.php'>登录页面</a>.</p>
