...
if (isset($_POST['username'])) {
  $colname1_rs2 = $_POST['username'];
}
$colname2_rs2 = "-1";
if (isset($_POST['question'])) {
  $colname2_rs2 = $_POST['question'];
}
$colname3_rs2 = "-1";
if (isset($_POST['answer'])) {
  $colname3_rs2 = $_POST['answer'];
}
mysql_select_db($database_conn, $conn);
$query_rs2 = sprintf("SELECT * FROM `user` WHERE username = %s and question = %s and answer = %s", GetSQLValueString($colname1_rs2, "text"),GetSQLValueString($colname2_rs2, "text"),GetSQLValueString($colname3_rs2, "text"));
...
<script>
  a = "Website:不会真有人忘记密码吧！！！";
  for (var i = 0; i < 5; i++) {
    console.log(a);
  }
  console.log("密码可是很重要的，不要再有下次了~");
</script>
...
<li><a href="../files/contribution.php<?php echo $url; ?>">投稿</a></li>
  ...
  <table width="500" border="0" cellspacing="0" cellpadding="5">
    <tr>
      <td width="136">你的用户名是：</td>
      <td width="344"><?php echo $row_rs2['username']; ?></td>
      </tr>
    <tr>
      <td>你的安全问题是：</td>
      <td><?php echo $row_rs2['question']; ?></td>
      </tr>
    <tr>
      <td>你的答案是：</td>
      <td><?php echo $row_rs2['answer']; ?></td>
      </tr>
    <tr>
      <td>你的密码是：</td>
      <td><?php echo $row_rs2['password']; ?></td>
      </tr>
  </table>
  <h4>恭喜您成功找回密码，请<a href="login.php">返回登录</a></h4>
  </div>
  <img src="../images/don'tFoget ThePasswd.jpg" width="200" title="Canton Tower：不要再忘记密码了哟，亲~">
      <?php } // Show if recordset not empty ?>
    <?php if ($totalRows_rs2 == 0) { // Show if recordset empty ?>
      <p>用户名错误或问题答案错误!找回密码失败。<br><a href="findPwd.php">返回</a></p>
      <?php } // Show if recordset empty ?>
	  <!-- InstanceEndEditable -->
  <!-- end .content --></div>
  <div class="footer">
